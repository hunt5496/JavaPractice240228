...
public class Ex_2_Variable {
	public static void main(String[] args) {
		...
		// 2. 유니 코드, 한글 사용 가능하지만, 잘 사용 안함.
		// 3. 자바에서 예약어(키워드) 변수 이름으로 사용 불가.
		// 4. 길이 제한은 없지만, 의미 있게 이름 짓는다.
		
		// 숫자 시작 안됨. 보여주고 주석합니다. ex) int 12d;
		int 학번;
		// 예약어는 컴파일러 오류가(문법 검사) 남.
		//String class;
//		int !num;
		...
		// 각 정수형의 데이터 범위의 구간을 굳이 다 외울 필요없다.
		// 10진수가 뭔지, 2진수가 뭔지 구분만 해주면 될듯.
		// 예) 10진수, 10, 이진수 : 00001010
		// byte byte2 = 128;
		
		// 자바에서 데이터 타입, 1) 기본형 8개 2) 참조형
		// byte 1바이트, short 2바이트, int 4바이트, long 8바이트 : 정수형
		// float, Double : 실수형, 지수와 가수 어려운 이야기,
		// 소숫점으로 정밀하게 측정해야하는 데이터 타입 사용이 된다.
		// 공학에 관련된 내용, 빅데이터 관련 통계에서 정확도를 측정 시 사용된다.
		// char, boolean 
		
		// 기본형 데이터 타입을 제외한, 나머지 타입이 레퍼런스 타입이다. (클래서, 인터페이스, 배열 등)
		...
		// lsyPerson : 참조형 변수(래퍼런스 변수), 클래스에서, 인스턴스(객체)를 생성했다.
		...
		// null, 아무것도 없다라고 표현하는데, 레퍼런스가 할당이 안되었다.
		// 기본형에서 사용 안함.
...
//		int testNumber = null;
		...
		// 큰 집 -> 작은 집으로 갈 때 , 문제가 있고, 값의 손실이 발생한다. 기본형.
		byte byteNumber1 = 10;
		long longNumber1 = byteNumber1;
		System.out.println("타입 변환 테스트 작은 집 -> 큰 집 갈 경우 : " + longNumber1);
		
		// 큰 집 -> 작은 집 갈 경우. 문제가 발생, 문법 검사기 걸렸음.
		// 강제로 형을 변환하기 위해서, 연산자, 캐스팅 연산자, 예) (강제로 변환할 형) + 변수
		// 개념적인 이야기이고, 실무에서는 사용을 안한다.
		int intNumber2 = 130;
		byte byteNumber2 = (byte)intNumber2;
		System.out.println("타입 변환 테스트 큰 집 -> 작은 집 갈 경우 강제로 형변환 ");
		System.out.println("byteNumber2 : " + byteNumber2);
		
		//레퍼런스에서는 , 포함관계로 설명을 하는데, 상속에 대해 설명 후 구체적으로 하기.
