...
public class Ex_6_Array_test {
	...
	public static void main(String[] args) {
		// 배열, 크기, 고정이고, 변경이 안됨.
		// 비유, 계란판 비유. 같은 타입의 데이터만 담을 수 있다.
		// 배열 생성 방법.
		// 1) int [] array ; 선언만
		// 1)-2 array = new in[4] ; , 선언 후 배열 생성(초기화를 한다 표현함.)
		// 1)-3 int [] array2 = new int [5]
		// 1)-4 int array2 [] = new int [5]
		// 2) int array3 [] ={1,2,3}
		// 인덱스 시작, 0부터 시작, 순서가 있다.
		// 선언 시 int array [4] ; , 크키를 지정하면 안됨.
		int array [] = new int[5];
		array[0] = 0;
		array[1] = 1;
		array[2] = 2;
		...
		for(int i = 0; i<arrayLunchMenuString.length; i++) {
			System.out.println("먹고싶은 메뉴는 : " + arrayLunchMenuString[i] + "입니다.");
		}
