...
public class Ex_03_Thread_class_with_UI_Event extends JFrame implements Runnable {
	...
	// JFrame 상속 받아서, 자바에서 제공해주는 Swing 패키지 안에 있는 UI 요소를 사용할 예정.
	// 생성자를 이용을 해서, 창을 그리기 준비.
	Ex_03_Thread_class_with_UI_Event(){
		// 창의 제목 설정.
		setTitle("클릭 시 창이 진동하는 것처럼 보이기, 스레드 연습");
		// 창 닫을 때, 종료하기.
	...
	public void run() {
		...
		// random.nextInt() -> 랜덤한 숫자를 의미하고, 이 숫자를 % 로 나누기.
		// 가능한 숫자 범위 : 0 ~ 4
		int x = getX() + random.nextInt()%5; // 새로운 x의 위치
		int y = getY() + random.nextInt()%5; // 새로운 y의 위치
