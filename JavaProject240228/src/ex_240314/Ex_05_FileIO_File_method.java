...
public class Ex_05_FileIO_File_method {
	...
	// 해당 디렉토리의 정보를 조회를 할려면, 반드시 , 해당 폴더의 경로를 알아야 한다.
	// 앞에서는 경로를 절대 경로를 문자열로 계속 전달했고,
	// 이제는 File 클래스에 경로를 담아서 매개변수에 정의.
	// 그러면 , 궁금한 폴더 경로의 File 인스턴스만 전달하면, 해당 File 인스턴스로
	// 정보 등을 조회가 가능하다.
	
	// 활용은 예) 프로필 이미지 등 , 처리할 때, File 클래스 이용해서 , 해당 폴더 및 파일 등을 임시로 생성 등 활용.
	// 현재 , 개발 연습중이니 , 로컬(윈도우즈경로) -> 나중엔 리눅스 경로에서도 동일하게 적용하면 됨.
	
	
	public static void listDirectory(File dir) {
		System.out.println("입력된 dir의 정보 조회 메서드=============");
		System.out.println("File dir의 인스턴스의 기능 중에서 getPath() : " + dir.getParent());
		// listFiles(), 본인 폴더 및 하위 폴더 등의 정보를 File 타입으로 배열로 리턴.
		File [] subFiles = dir.listFiles();
		
		// 하위 정보를 바로 사용이 안되니, 한번 더 File 타입의 기능을 이용해서, 필요한 정보들 출력해보기.
	...
	public static void main(String[] args) {
		...
		// 파일 생성, 폴더 생성, 파일의 메타 정보 등을 확인 가능.
		// 직접적인 입출력 불가.
		...
		// 내가 사용한 파일 외에, 시스템에 제공하는 기본 파일 등이 있음.
		// 그래서, 결론 , 오류코드가 보이면, 먼저) 내가 작업한 파일이 있는지 여부를 확인.
		
		// 파일 인지, 디렉터리 인지 유무 확인, 디렉터리 생성도 확인.
		if (f1.isFile()) {
			System.out.println("f1 은 파일입니다.");
		} else {
			System.out.println("f1 은 폴더입니다.");
		}
