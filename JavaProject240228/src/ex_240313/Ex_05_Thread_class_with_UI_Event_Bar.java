...
//	응용 순서 1, 수치 라벨을 붙이기 위한 인스턴스 필요함.
	JLabel sizeBarLabel = new JLabel();
	
	
	// 응용 순서 2, 수치 라벨의 인스턴스를 , MyLabel에서 스레드로, 바크기가 맞게 그려지고 있음.
	// sizeBarLabel 인스턴스를, MyLabel에 던졌음. 보냈다.
	// 왜 보냈는가? 스레드가 동작 중인데, MyLabel 동작중, 매번 barSize의 크기가 지정이 되니,
	// MyLabel 의 크기를 -> sizeBarLabel , 크기 값을 전달을 했음.
	// call by value 는 메소드 안에서 값이 사용 후 휘발, 사라지지만,
	// 여기서 인스턴스를 사용해서, 계속 그 인스턴스를 유지 함.
